...
Beanstream.prototype.submitTokenTransaction = function submitTokenTransaction (order, token, prospect, other) {
  var self = this;

  var body = assign({
    amount: +((+(order.amount)).toFixed(2)),
    payment_method: 'card',
    comments: 'Order processed through 42-cent-beanstream with token',
    billing: mapKeys(prospect, billingSchema),
    shipping: mapKeys(prospect, shippingSchema),
    token: mapKeys(token, tokenSchema)
  });
  console.log(JSON.stringify(body, null, 4));
  return self._post('/payments', assign(body, other || {}));
};
